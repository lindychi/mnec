...
def view(request, user_name, todo_name):
    if request.user.is_authenticated and request.user.username == user_name:
        todo = Todo.objects.get(owner=request.user, title=todo_name)
        return render(request, 'nec_todo/view.html', {'user_name':user_name, 'todo':todo})
    else:
        return redirect(settings.LOGIN_URL)


def edit(request, user_name, todo_name):
    if request.user.is_authenticated and request.user.username == user_name:
        todo = Todo.objects.get(title=todo_name, owner=request.user)
        return render(request, 'nec_todo/edit.html', {'todo':todo})
    else:
        return redirect(settings.LOGIN_URL)


def delete(request, user_name, todo_name):
    return None
